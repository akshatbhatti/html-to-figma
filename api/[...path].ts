// Vercel serverless entrypoint AND the single definition of the API.
//
// Everything lives in this one file on purpose. Vercel compiles each function
// entrypoint with TypeScript but does NOT follow relative TypeScript imports: it
// emitted /var/task/api/[...path].js whose `import '../server/app'` resolved to
// nothing, and the function died at load with ERR_MODULE_NOT_FOUND before any of
// our code ran. npm packages trace and bundle correctly; only local files do not.
// So the function imports node_modules and nothing else, and there is nothing
// left for the platform to fail to resolve.
//
// The route is a catch-all ([...path]) because Vercel maps api/index.ts to exactly
// `/api`; the catch-all receives /api/login, /api/fetch-url, … with the path intact.
//
// server.ts (local dev only) imports the `app` export below, so there is still one
// definition of the API shared by both environments.
...
/**
 * Vercel invokes this. Express handles its own errors, but a throw here would
 * otherwise surface as an opaque FUNCTION_INVOCATION_FAILED page with no cause,
 * so failures are reported as JSON and logged to the Runtime Logs.
 */
